...
if __name__ == "__main__":
    ...
    logger.info(
        "FFT will be computed on %d second epoch in the buffer with an overlap of %f seconds",
        EPOCH_LENGTH,
        OVERLAP_LENGTH,
    )
    ...
    eeg_buffer, filter_state = utils.populate_initial_buffer(
        inlet, eeg_buffer, filter_state, SHIFT_LENGTH, fs, INDEX_CHANNEL
    )
    ...
    try:
        while True:
            ...
            band_powers = np.array(
                [
                    utils.compute_band_powers(data_epoch[:, ch].reshape(-1, 1), fs)
