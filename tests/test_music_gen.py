...
def test_generate_next_event(meta_generator):
    ...
    chord_event = meta_generator.generate_next_events(valence, arousal)
    
    assert chord_event is not None
    assert chord_event[0].velocity > 64  # Higher velocity with high arousal
...
def test_generate_modal_midi_vamp(meta_generator):
    ...
    assert all(event.duration == 8 for event in chord_events)
