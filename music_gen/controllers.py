...
class AbletonOSCController:
    ...
    def set_tracks_volume(self, volume: float) -> None:
        """Set volume of all 4 tracks, oscillates between 0.5 and .9"""
...
class AbletonMetaController:
    ...
    def update_valence(self, valence: float) -> None:
        ...
        # self._modulate_all(valence, self.arousal)
    ...
    def update_arousal(self, arousal: float) -> None:
        ...
        self._modulate_bass(self.valence, arousal)
        # self._modulate_all(self.valence, arousal)
    ...
    def add_events_to_ableton(self, start_bar_num:int) -> None:
        ...
        # piano 
        self.controller.remove_and_add_notes(0, 0, midi_events[0].to_ableton_osc(start_time=start_bar_num), start_bar_num)
        # # bass, only the root note
        self.controller.remove_and_add_notes(2, 0, [int(midi_events[0].root-12), start_bar_num, midi_events[0].duration, midi_events[0].velocity, 0], start_bar_num)
        # # arpeggiator
    ...
    # def _modulate_all(self, valence: float, arousal: float) -> None:
    #     # self._modulate_arpeggiator(valence, arousal)
    #     self._modulate_bass(valence, arousal)
    #     # self.controller.song.set_tempo(80 + arousal * 40)  # oscillates between 80 and 130
    #     # self.controller.set_tracks_volume(0.5 + arousal * 0.4)  # oscillates between 0.6 and .9
